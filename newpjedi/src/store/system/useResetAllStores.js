...
//resetters: Đây là một mảng trống ban đầu được sử dụng để lưu trữ các hàm reset trạng thái của các store. Mỗi hàm trong mảng 
//này sẽ được gọi khi chúng ta muốn đặt lại (reset) trạng thái của tất cả các store về trạng thái ban đầu.

//createStore: Đây là một hàm tạo store, nhận vào một hàm f là hàm tạo store từ thư viện zustand. Trong hàm này:

//Nếu f không được định nghĩa (undefined), hàm trả về chính nó (createStore). Điều này có thể được sử dụng khi gọi hàm createStore 
//mà không cần truyền vào hàm tạo store.

//Nếu f được định nghĩa, hàm tạo store create(f) được gọi để tạo ra một store mới. Sau đó, trạng thái ban đầu của store được lấy ra 
//và lưu vào biến initialState. Hàm resetters.push() được sử dụng để thêm một hàm reset trạng thái của store vào mảng resetters. 
//Hàm này sẽ thiết lập trạng thái của store về trạng thái ban đầu mỗi khi được gọi.
